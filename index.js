// iCost · 我的账本  v7.1  — minimalist MUJI style · resilient
(function () {
  'use strict';

  // ── 防御性包装：确保任何内部错误都不影响酒馆正常运行 ────────────
  function safe(fn) {
    return function () {
      ...
      <div id="ic-body">
        ...
        <div class="ic-rechd" id="ic-rechd">
          ...
          <div class="ic-rechd-right">
            <span class="ic-cnt" id="ic-cnt">0 条记录</span>
  ...
  /* ── render logic ────────────────────────────────── */
  const renderTags = safe(function(){
  ...
  const render = safe(function(){
    ...
    const bwrap=document.getElementById('ic-budget-wrap'), bunset=document.getElementById('ic-budget-unset');
    if(bwrap&&bunset){
      if(!budget){ bwrap.style.display='none'; bunset.style.display='flex'; }
      else {
        bwrap.style.display='flex'; bunset.style.display='none';
        const pct=Math.min(exp/budget*100,100), fill=document.getElementById('ic-budget-fill');
        if(fill){
          fill.style.width=pct+'%';
          fill.className='ic-budget-bar-fill '+(pct>=90?'ic-bud-red':pct>=70?'ic-bud-yellow':'ic-bud-green');
        }
        const lbl=document.getElementById('ic-budget-label');
        if(lbl) lbl.textContent=`Used: ¥${exp.toFixed(0)} / ¥${budget.toFixed(0)}`;
      }
    }
  ...
  const cancelEdit = () => {
    editingId=null;
    document.getElementById('ic-amt').value=''; document.getElementById('ic-note').value=''; curTag='';
    const btn=document.getElementById('ic-addbtn');
  ...
  const share = safe(() => {
    ...
    shareStep=0;
    if(ne) ne.style.display='none'; if(btn) btn.classList.remove('ic-share-confirm'); if(cb) cb.style.display='none';
    const recs=load(), inc=recs.filter(r=>r.type==='income').reduce((s,r)=>s+r.amount,0), exp=recs.filter(r=>r.type==='expense').reduce((s,r)=>s+r.amount,0);
  ...
  const injectPanel = safe(() => {
    ...
    sec.innerHTML=`<div class="ic-ext-row">
      <div class="ic-ext-info"><span class="ic-ext-name">iCost</span><span class="ic-ext-sub">Simple Minimalist Ledger</span></div>
      <button class="ic-ext-btn" id="ic-panel-toggle">打开账本</button>
    </div>`;
    target.prepend(sec);
    document.getElementById('ic-panel-toggle').addEventListener('click',()=>winVisible?closeWin():openWin());
  });

  const mount = safe(() => {
    if(document.getElementById('ic-win')) return;
    const w=buildWin(); document.body.appendChild(w);
    try{const p=JSON.parse(localStorage.getItem(POS_KEY)||'null');if(p&&p.l&&p.t){w.style.left=p.l;w.style.top=p.t;w.style.right='auto';w.style.bottom='auto';}}catch(e){}
    enableDrag(w);
    ...
    document.getElementById('ic-month-prev').addEventListener('click',e=>{ e.stopPropagation(); curMonth=prevMonth(curMonth); render(); });
    document.getElementById('ic-month-next').addEventListener('click',e=>{ e.stopPropagation(); curMonth=nextMonth(curMonth); render(); });
    ...
    document.getElementById('ic-share-cancel').addEventListener('click',()=>{
      shareStep=0; const ne=document.getElementById('ic-share-note'),btn=document.getElementById('ic-share'),cb=document.getElementById('ic-share-cancel');
      if(ne){ne.style.display='none';ne.value='';} if(btn){btn.textContent='📤 发送账单';btn.classList.remove('ic-share-confirm');} if(cb) cb.style.display='none';
    });
    const openBI=()=>{
      const cur=loadBudget(); const val=prompt(`设置月度支出预算\n当前：${cur?'¥'+cur:'未设置'}`,cur||'');
      if(val!==null){ const num=parseFloat(val); saveBudget(!isNaN(num)&&num>0?num:0); render(); }
    };
    document.getElementById('ic-budget-set-btn').addEventListener('click',e=>{ e.stopPropagation(); openBI(); });
    document.getElementById('ic-budget-set-btn2').addEventListener('click',e=>{ e.stopPropagation(); openBI(); });
    renderTags();
  });

  // ── init — 三重延迟挂载，绝不阻塞页面加载 ───────────────
  const init = safe(() => {
    mount();
    injectPanel();
  });
