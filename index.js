...
async function fetchModels() {
  ...
  try {
    ...
    const res  = await fetch(`${base}/v1/models`, {headers:{'x-api-key':apiConfig.apiKey,'anthropic-version':'2023-06-01'}});
    if (!res.ok) return [];
...
function renderModelSection(models) {
  ...
  if (models.length>0) {
    ...
    wrap.innerHTML=`<div class="ins-field"><label>使用模型</label><select id="ins-model-sel">${opts}</select></div>`;
    document.getElementById('ins-model-sel').addEventListener('change',e=>{apiConfig.model=e.target.value;ls(KEY.api,apiConfig);});
  } else {
    wrap.innerHTML=`<div class="ins-field"><label>模型名称（手动填）</label><input id="ins-model-input" type="text" placeholder="claude-sonnet-4-20250514" value="${esc(apiConfig.model||'')}"><div class="ins-note">未返回列表，手动填写</div></div>`;
...
function buildWin() {
  ...
  <div id="ins-body">
    ...
    <div id="ins-inputbar">
      <button id="ins-persona-btn" class="${inputPersona}">${inputPersona==='daddy'?'D':'P'}</button>
  ...
  document.getElementById('ins-api-save').addEventListener('click', async ()=>{
    ...
    btn.disabled=false;
    btn.textContent=models.length>0?'✓ 已连接':'保存并连接';
    if(models.length>0) setTimeout(()=>{btn.textContent='保存并连接';},2000);
  ...
  document.getElementById('ins-persona-btn').addEventListener('click',()=>{
    inputPersona=inputPersona==='pearl'?'daddy':'pearl';
    const btn=document.getElementById('ins-persona-btn');
