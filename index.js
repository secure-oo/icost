...
function avatarHtml(who, size) {
  ...
  if (src) {
    return `<div style="${style}"><img src="${esc(src)}" alt="${esc(name)}" style="width:100%;height:100%;object-fit:cover;"></div>`;
  }
  const letter = (name || '?')[0].toUpperCase();
  const bg = isA ? '#e8e8e8' : '#f0f0f0';
  return `<div style="${style}background:${bg};font-size:${Math.floor(sz*0.45)}px;font-weight:600;color:#555;">${esc(letter)}</div>`;
}
...
function pickAvatar(who) {
  ...
  input.onchange = () => {
    ...
    reader.onload = e => {
      ...
      if (el) el.innerHTML = avatarHtml(who, 72);
      rebindProfileClicks();
      renderMessages();
    };
...
function updateExtBtn(open) {
  const btn = document.getElementById('pb-ext-toggle-btn');
  if (!btn) return;
...
function openWin() {
  const w = document.getElementById('pb-dm-modal'); if (!w) return;
  winVisible = true;
  w.style.display = 'flex';
  requestAnimationFrame(() => w.classList.add('pb-visible'));
  renderMessages();
  updateExtBtn(true);
}
...
// ── 悬浮球（全 inline style，挂到 <html> 根节点）────────
function buildTriggerButton() {
  ...
  // 挂到 <html> 根节点，绕开 body transform 导致 fixed 失效的问题
  (document.documentElement || document.body).appendChild(btn);
}

// ── 扩展面板注入 ──────────────────────────────────────────
function injectExtPanel(container) {
  if (document.getElementById('pb-ext-section')) return;
  const sec = document.createElement('div');
  sec.id = 'pb-ext-section';
  ...
  sec.innerHTML = `
    <span style="display:flex;align-items:center;gap:8px;font-size:13px;font-weight:600;color:#111;">
      ${SVG.chat.replace('stroke="white"','stroke="#111"')}
      珍珠湾 DM
    </span>
    <button id="pb-ext-toggle-btn" style="
  ...
  document.getElementById('pb-ext-toggle-btn').addEventListener('click', toggleWin);
}

function tryInjectPanel() {
  if (document.getElementById('pb-ext-section')) return true;
  // iCost 同款：只用 #extensions_settings，它在桌面和手机端都存在
  const target = document.getElementById('extensions_settings');
  if (target) { injectExtPanel(target); return true; }
  return false;
}

// ── 构建主窗口（直接 position:fixed，无 overlay）────────
// 关键：不用 overlay 遮罩层，ST 手机版 body 有 transform 会让 fixed 遮罩失效
function buildModal() {
  if (document.getElementById('pb-dm-modal')) return;

  const modal = document.createElement('div');
  modal.id = 'pb-dm-modal';
  ...
  // 挂到 <html> 根节点，彻底绕开 body transform 问题
  (document.documentElement || document.body).appendChild(modal);

  // ── 关闭按钮
  document.getElementById('pb-modal-close').addEventListener('click', closeWin);
...
// ── 初始化 ── iCost 同款三重延迟策略 ───────────────────────
function safe(fn) {
  return function() { try { return fn.apply(this, arguments); } catch(e) { console.warn('[珍珠湾]', e); } };
}
...
// MutationObserver 兜底：ST 面板懒加载时补注入
const obs = new MutationObserver(safe(() => {
  if (!document.getElementById('pb-ext-section')) tryInjectPanel();
}));
