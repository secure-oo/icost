...
function init(){
  ...
  buildWin();
  buildFloatBtn();
  // 扩展面板：找到就停，找不到用轻量observer等待
  if(!tryInject()){
    let obs=null;
    const stopObs=()=>{if(obs){obs.disconnect();obs=null;}};
    obs=new MutationObserver(safe(()=>{
      if(tryInject()) stopObs();
    }));
    // 只监听 body 直接子节点，不用 subtree，手机不卡
    if(document.body) obs.observe(document.body,{childList:true});
    setTimeout(stopObs,7000);
  }
}

// iCost 同款三重延迟，让 ST 先加载完
