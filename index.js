// iCost · 我的账本  v6.2  — mobile UX + dot badge + share reveal
(function () {
  'use strict';
  ...
  let editingId  = null;
  let shareStep  = 0;   // 0=hidden note, 1=note revealed waiting confirm
      ...
      <div id="ic-body">
        <div class="ic-summary">
          <div class="ic-sum-bal">
            ...
            <span class="ic-sval ic-green ic-bal-big" id="ic-bal">¥0.00</span>
          </div>
  ...
  function render(){
    ...
    bel.className='ic-sval ic-bal-big '+(bal>=0?'ic-green':'ic-red');
    ...
    list.innerHTML=[...recs].sort((a,b)=>b.date.localeCompare(a.date)).map(r=>`
      <div class="ic-row ic-row-${r.type}" data-id="${r.id}">
        ...
        <div class="ic-row-content">
          <div class="ic-rl">
            <div class="ic-type-dot ic-dot-${r.type}">${r.type==='income'?'I':'E'}</div>
  ...
  function cancelShare(){
    shareStep = 0;
    const noteEl  = document.getElementById('ic-share-note');
    const btn     = document.getElementById('ic-share');
    const cancelBtn = document.getElementById('ic-share-cancel');
    noteEl.style.display = 'none';
    noteEl.value = '';
    btn.textContent = '📤 发给他看';
    btn.classList.remove('ic-share-confirm');
  ...
  function share(){
    ...
    // Step 1: reveal note input + confirm/cancel
    if(shareStep === 0){
      noteEl.style.display = 'block';
      noteEl.focus();
      btn.textContent = '✓ 确认发送给他';
      btn.classList.add('ic-share-confirm');
    ...
    setTimeout(()=>{ btn.textContent='📤 发给他看'; btn.classList.remove('ic-share-ok'); },1800);
  }
