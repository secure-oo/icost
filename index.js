...
function buildTriggerButton() {
  ...
  // 挂到 <html> 根节点，避免被 body 的 overflow 或层叠上下文裁切（移动端常见问题）
  (document.documentElement || document.body).appendChild(btn);

  // ── 移动端额外兜底：监听 ST 扩展按钮点击，让窗口随之弹出
  // （用于悬浮球确实不可见的场景）
  const stExtBtnSels = [
    '#extensionsMenuButton', '#btn-extensions',
    '.drawer-btn[data-type="extensions"]',
    '#option_toggle_extensions',
    'button[title*="扩展"]', 'button[title*="Extension"]',
    '#extensionMenuHeaderButton',
  ];
  const bindSTBtn = () => {
    for (const sel of stExtBtnSels) {
      const el = document.querySelector(sel);
      if (el && !el._pbBound) {
        el._pbBound = true;
        el.addEventListener('click', () => {
          // 等面板渲染完再注入
          setTimeout(() => { tryRegisterSTExtension(); }, 80);
        });
      }
    }
  };
...
function injectSTToggle(container) {
  ...
  row.innerHTML = `
    ...
    <span style="font-size:11px;color:#aaa;font-weight:400;">点击打开 →</span>`;
  ...
  // 手机端触摸反馈
  row.addEventListener('touchstart', () => { row.style.background = '#ececec'; }, { passive: true });
  row.addEventListener('touchend',   () => { row.style.background = '#f8f8f8'; }, { passive: true });
...
function tryRegisterSTExtension() {
  ...
  const sels = [
    ...
    '#extensionsMenu',
    '#ExtensionList',
    '#extensionsMenuExtensionList',
    '.drawer-content[data-type="extensions"]',
    '.drawer-content',
    '#right-nav-panel',
    '.right-panel',
    '#right_panel',
    '#option_toggle_extensions',   // ST mobile top bar
    '.options-content',
    '#options',
  ];
  ...
  for (const sel of sels) {
    const el = document.querySelector(sel);
    if (el && el.offsetParent !== null) { injectSTToggle(el); return true; }
  }

  // 兜底：如果都找不到，就把行注入到 body 最前面（保证在移动端至少有一个入口）
  const fallback = document.getElementById('pb-st-toggle-row');
  if (!fallback) {
    // 不注入 body，等 observer 继续找
  }
  return false;
}
...
(function init() {
  ...
  function go() {
    ...
    if (!tryRegisterSTExtension()) {
      ...
      // 移动端：延迟多次补充尝试（ST 可能懒加载面板）
      [300, 800, 1500, 3000].forEach(delay => {
        setTimeout(() => tryRegisterSTExtension(), delay);
      });
    }
  }
