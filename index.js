...
function tryRegisterSTExtension() {
  ...
  for (const sel of selectors) {
    ...
    if (menu) {
      // 避免重复注册
      if (document.getElementById('pb-st-entry')) return true;
  ...
  // ST extension_settings 顶部注入 toggle 开关
  const settingsSelectors = ['#extension_settings', '.extension_settings'];
  for (const sel of settingsSelectors) {
    const panel = document.querySelector(sel);
    if (panel && !document.getElementById('pb-st-toggle-row')) {
      const row = document.createElement('div');
      row.id = 'pb-st-toggle-row';
      row.style.cssText = 'display:flex;align-items:center;justify-content:space-between;padding:10px 14px;border-bottom:1px solid #efefef;font-size:13px;font-weight:600;';
      row.innerHTML = `
        <span style="display:flex;align-items:center;gap:7px;">
          ${SVG.chat.replace('stroke="white"','stroke="currentColor"')}
          <span>珍珠湾 DM</span>
        </span>
        <label style="position:relative;display:inline-block;width:36px;height:20px;cursor:pointer;">
          <input type="checkbox" id="pb-st-chk" style="opacity:0;width:0;height:0;" />
          <span style="position:absolute;inset:0;background:#ddd;border-radius:20px;transition:.2s;" id="pb-st-track"></span>
          <span style="position:absolute;top:2px;left:2px;width:16px;height:16px;background:#fff;border-radius:50%;transition:.2s;box-shadow:0 1px 3px rgba(0,0,0,.2);" id="pb-st-thumb"></span>
        </label>`;
      ...
      chk.addEventListener('change', () => {
        ...
        if (on) { overlay?.classList.add('open'); renderMessages(); }
