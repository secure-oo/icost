...
function renderMessages() {
  ...
  // 删除按钮：桌面hover显示，手机长按显示
  let shownDel = null;
  container.querySelectorAll('.ins-bw').forEach(bw => {
    let lpt;
    bw.addEventListener('touchstart', () => {
      lpt = setTimeout(() => {
        if (shownDel && shownDel !== bw) shownDel.querySelector('.ins-del').style.display = '';
        const d = bw.querySelector('.ins-del');
        if (d) { d.style.display = 'flex'; shownDel = bw; }
      }, 500);
    }, {passive:true});
    bw.addEventListener('touchend',  () => clearTimeout(lpt), {passive:true});
    bw.addEventListener('touchmove', () => clearTimeout(lpt), {passive:true});
  });
